...
#
#環境設定
#メールの接続テストは、「sample_settings_mail.txt」内を設定し（[]内に記述）、コマンドプロンプトで、例：python.exe c:/muma_miru/sample_send_mail.pyを実行して、「Sent mail successfully.」が表示されるか確認して下さい。
...
#テキストファイルを作成後、python.exe 「ファイルのパス」/kuma_miru_mail.pyを記述し、ファイルの拡張子を.batに変更して下さい（バッチファイルの作成）。
#ファイル名の例：kuma_miru.bat
#記述の例：python.exe c:/muma_miru/kuma_miru_mail.py
...
#スタートアップフォルダへの登録方法3：先程作成したkuma_miru_mail.batをスタートアップフォルダに移動します。
