...
#アマゾン社で購入出来る安価なPC等で運用可能です（INTEL社N97やN95で処理時間は約80ミリ秒×カメラの台数となり、PC本体とカメラ１台の費用は3万円前後となります）（AMD社のRyzen7PRO6850Hの処理時間は約90ミリ秒×カメラの台数となり、PC本体とカメラ1台の費用は6万円前後となりますが、ほぼリアルタイムな処理が可能となります）
#
#環境設定
#必要に応じて、設定ファイル「sample_settings_sound.txt」を編集して下さい。
...
#コマンドプロントで、pip install pygameを実行してください。
#テキストファイルを作成後、python.exe 「ファイルのパス」/sample_kuma_miru_sound.pyを記述し、ファイルの拡張子を.batに変更して下さい（バッチファイルの作成）。
#ファイル名の例：sample_kuma_miru_sound.bat
#記述の例：python.exe c:/muma_miru/sample_kuma_miru_sound.py
...
#スタートアップフォルダへの登録方法3：先程作成したsample_kuma_miru_sound.batをスタートアップフォルダに移動します。
#パソコン起動後に本プログラムも自動で起動します。
#
#Ultralytics社のモジュールを使用する事で、簡単にOpenVINOを使用した、高速な物体検出が可能となりました。
...
#
#本プログラムで使用する犬の効果音データは、「効果音ラボ」様のフリー効果音素材を使用させて頂きました。
...
#検出時に効果音を再生する実装関連1（環境に合わせて設定を変更）=============----------オリジナルに追加----------
import pygame #音声を再生するモジュールの読み込み
import random #乱数を生成するモジュールの読み込み
print("Reading settings.") #設定ファイルを読み込んでいる事を表示----------オリジナルに追加----------
file = open("sample_settings_sound.txt", "r", encoding="utf-8") #sample_settings_sound.txtファイルの読み込み----------オリジナルに追加----------
settings = file.readlines() #ファイルの各行をリストに一括読み込み----------オリジナルに追加----------
file.close() #ファイルの読み込み終了----------オリジナルに追加----------
number_of_soundfiles = int(value(settings[0])) #効果音のファイル数（0.mp3から開始）----------オリジナルに追加----------
number_of_play = int(value(settings[1])) #検出した際に、同じ効果音群を何回再生する指定----------オリジナルに追加----------
process_waiting_time = int(value(settings[2])) #効果音再生中、処理の実行を何秒待つか指定（0はオフ）----------オリジナルに追加----------
play_random = int(value(settings[3])) #効果音ファイルをランダムに再生するか指定（0はオフ）----------オリジナルに追加----------
play_interval = int(value(settings[4])) #効果音ファイルする間隔を秒単位で指定（0はオフ）----------オリジナルに追加----------
#====================================================================----------オリジナルに追加----------


#検出時にっ効果音を再生する実装関連2（変更不可）=========================----------オリジナルに追加----------
timer_sound_start = 0 #設定した間隔でメールを送信する為のタイマー用（開始の時間）----------オリジナルに追加----------
timer_sound_current = 0 #設定した間隔でメールを送信する為のタイマー用（現在の時間）----------オリジナルに追加----------
timer_sound_elapsed = 0  #設定した間隔でメールを送信する為のタイマー用（経過時間）----------オリジナルに追加----------
pg = [] #各効果音の設定を格納するリスト----------オリジナルに追加----------
pygame.init() #効果音を再生するモジュールを初期化----------オリジナルに追加----------
for i in range(number_of_soundfiles): #効果音の数、処理を実行----------オリジナルに追加----------
    l=pygame.mixer.Sound(str(i) + ".mp3") #効果音を読み込み----------オリジナルに追加----------
    pg.append(l) #効果音の設定をリストに格納----------オリジナルに追加----------
#====================================================================----------オリジナルに追加----------
...
#model_type = "yolo11n_openvino_model/"
model_type = "yolo11s_openvino_model/"
#model_type = "yolo11m_openvino_model/"
#model_type = "yolo11l_openvino_model/"
#model_type = "yolo11x_openvino_model/"
weights = ["yolo11n", "yolo11s","yolo11m", "yolo11l", "yolo11x"] #変換する学習済みファイル名をリストに格納（拡張子なし）
#====================================================================


#オリジナルの学習済みモデルをOpenVINO形式に変換==========================
for i in weights: #各変換前のファイル名をリストから取得
    if os.path.isfile(i + "_openvino_model/metadata.yaml") and os.path.isfile(i + "_openvino_model/" + i + ".bin") and os.path.isfile(i + "_openvino_model/" + i + ".xml"): #指定したファイルがあるか確認
        print(i + " dataset for OpenVino found.") #指定ファイルがある事を表示
    else: #指定したファイルが無かった場合
        if os.path.isfile(i + ".pt"): #指定したファイルがあるか確認
            model = YOLO(i + ".pt") #学習済みモデルの読み込み
            model.export(format="openvino") #学習済みモデルの変換
            print("Converted " + i + ".pt to OpenVINO format.") #変換が成功した事を表示
        else: #指定したファイルが無い場合
...
while(True): #繰り返し処理
    if timer_sound_start == 0: #タイマーが開始していない場合----------オリジナルに追加----------
        timer_sound_start = time.time() #タイマーの開始時間を取得----------オリジナルに追加----------
    ...
    if elapsed_time >= timer_interval: #経過時間が、設定した時間より大きいか確認
        ...
        if external_output == 1: #外部出力処理（#####外部出力処理は未実装#####）
            external_output = 0 #外部機器に出力（オフ）
            timer_sound_current = time.time() #現在の時間を取得----------オリジナルに追加----------
            timer_sound_elapsed = int(timer_sound_current - timer_sound_start) #経過時間を取得（整数に変換）----------オリジナルに追加----------
            if timer_sound_elapsed >= play_interval: #経過時間が、設定した時間より大きいか確認----------オリジナルに追加----------
                timer_sound_start = 0 #タイマーをリセット----------オリジナルに追加----------
                for i in range(number_of_play): #指定した回数、同じ効果音群を何回再生----------オリジナルに追加----------
                    if play_random == 0: #ランダムに効果音を再生しない場合----------オリジナルに追加----------
